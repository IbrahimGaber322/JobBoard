...
<template>
    <div v-if="buttons.length > 1">
        <div class="flex flex-wrap justify-center">
            <Link class="px-4 py-2 ml-2 mr-4 text-sm rounded-lg" :class="button_prev_url
            ...
            <template v-for="(btn, i) in buttons" v-bind:key="i">
                <Link class="px-4 py-2 mx-0.5 text-sm font-bold rounded-lg hover:bg-main-50 focus:text-main-500" :class="btn.active ? 'bg-main-600 text-blue-500' : 'text-slate-500'
                    " :href="btn.url">
                {{ btn.label }}
                </Link>
            </template>
            <Link class="px-4 py-2 ml-4 mr-2 text-sm rounded-lg" :class="button_next_url
                ? 'text-slate-500 focus:text-main-500 hover:bg-main-50 font-bold'
                : 'text-slate-300'
