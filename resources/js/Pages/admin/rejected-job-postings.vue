        ...
        <div class="relative bg-gradient-to-r">
            ...
            <div class="container mx-auto">
                <div class="flex justify-center">
                    <h1 class="text-4xl font-bold mt-5 text-blue-900">Rejected Job Postings</h1>
                </div>
                <div class="flex justify-center items-center">
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 mt-10 mb-10">
                        <div v-for="rejectedJob in rejectedJobPostings" :key="rejectedJob.id" class="relative">
                            <a :href="'/admin/job-postings/' + rejectedJob.id">
                                <div class="bg-white rounded-lg shadow-md p-6 w-full md:w-5/6 lg:w-8/10 jobCard cursor-pointer">
                                    <h2 class="text-lg font-semibold mb-4 text-blue-900">{{ rejectedJob.title }}</h2>
                                    <p class="text-gray-700 mb-4 description">{{ rejectedJob.desc }}</p>
                                    <div class="text-gray-700 mb-4">
                                        <p class="font-semibold text-blue-900">Category:</p>
                                        <p class="italic">{{ rejectedJob.category }}</p>
                                        <p class="font-semibold text-blue-900">Location:</p>
                                        <p class="italic">{{ rejectedJob.location }}</p>
                                    </div>
                                </div>
                            </a>
                        </div>
...
export default {
    ...
    approveJob(jobId) {
        ...
        .then(response => {
            if (response.ok) {
                this.rejectedJobPostings = this.rejectedJobPostings.filter(job => job.id !== jobId);
            } else {
